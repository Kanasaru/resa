...
class MainMenu(forms.Interface):
    def __init__(self, sheet_key):
        ...
        self.bg_image = RESA_CH.background_image
        self.__credits = f"{locales.get('info_credits')} {RESA_CH.author} | {RESA_CH.www}"
    ...
    def build(self):
        ...
        # labels
        tf_headline = forms.Label((int(self.title.width() / 2), 20), RESA_CH.title.upper(), 90)
        tf_headline.align(forms.CENTER)
        tf_version = forms.Label((self.title.width() - 5, 5), f'v{RESA_CH.version}', 14)
        ...
        b_newgame = forms.Button(
            ...
            pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_STARTGAME)
        )
        ...
        b_loadgame = forms.Button(
            ...
            pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_LOADGAME)
        )
        b_loadgame.align(forms.CENTER)
        try:
            f = open(RESA_CH.save_file)
            f.close()
        ...
        b_options = forms.Button(
            ...
            pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_OPTIONS)
        )
        ...
        b_quitgame = forms.Button(
            ...
            pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_QUITGAME)
        )
        ...
        b_editor = forms.Button(
            ...
            pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_EDITOR)
        )
...
class Options(forms.Interface):
    ...
    def build(self):
        ...
        # labels
        tf_headline = forms.Label((self.title.width() // 2, 20), RESA_CH.title.upper(), 90)
        tf_headline.align(forms.CENTER)
        tf_version = forms.Label((self.title.width() - 5, 5), f'v{RESA_CH.version}', 14)
        tf_version.align(forms.RIGHT)
        ...
        if RESA_CH.fullscreen:
            for mode in self.screenmodes['full'][0:3]:
                btn = forms.Button(
                    ...
                    RESA_SSH, self.sheet_key,
                    f'{mode[0]}x{mode[1]}',
                    pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_CHG_RESOLUTION, res=mode)
                )
        ...
        if self.screenmodes['full']:
            ...
            swt_fullscreen = forms.Switch(
                ...
                pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.SWT_FULLSCREEN,
        ...
        b_back = forms.Button(
            ...
            pygame.event.Event(RESA_EH.TITLE_EVENT, code=RESA_EH.BTN_MAINMENU)
        )
...
MAINMENU = MainMenu('MenuButtons')
OPTIONSMENU = Options('MenuButtons')
