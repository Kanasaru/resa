...
class GridFieldNeighbors(object):
    def __init__(self):
        """ Dataclass for grid field neighbors. """
        self._left = False
        self._right = False
        self._top = False
        self._bottom = False
    ...
    def __str__(self):
        return f'LRTB({self.left}, {self.right}, {self.top}, {self.bottom}) | ' \
               f'TlTrBlBr({self.topleft}, {self.topright}, {self.bottomleft}, {self.bottomright})'
...
class Grid(object):
    ...
    def iso_grid_neighbors(self, key: int) -> GridFieldNeighbors:
        """ Checks if given field has neighbors and their keys.

        :param key: key of the isometric field
        :return: keys of fields neighbors
        """
        neighbors = GridFieldNeighbors()
        neighbors.left = key - 1
        neighbors.right = key + 1
        neighbors.top = key - (self.fields_x // 2) * 2 + 1
        neighbors.bottom = key + (self.fields_x // 2) * 2 - 1
        neighbors.topleft = key - self.fields_x // 2
        neighbors.topright = key - self.fields_x // 2 + 1
        neighbors.bottomleft = key + self.fields_x // 2 - 1
        neighbors.bottomright = key + self.fields_x // 2

        # check against left and right grid borders
        if self.fields_iso[key].col == 0 and self.fields_iso[key].row % 2 == 0:
            neighbors.left = -1
            neighbors.topleft = -1
            neighbors.bottomleft = -1
        if self.fields_iso[key].col == 0 and self.fields_iso[key].row % 2 != 0:
            neighbors.left = -1
        if self.fields_iso[key].col == self.fields_x // 2 - 1 and self.fields_iso[key].row % 2 == 0:
            neighbors.right = -1
            neighbors.topright = -1
            neighbors.bottomright = -1
        if self.fields_iso[key].col == self.fields_x // 2 - 2 and self.fields_iso[key].row % 2 != 0:
            neighbors.right = -1

        # check against bottom grid border
        if neighbors.bottom > len(self.fields_iso):
            neighbors.bottom = -1
        if neighbors.bottomleft > len(self.fields_iso):
            neighbors.bottomleft = -1
        if neighbors.bottomright > len(self.fields_iso):
            neighbors.bottomright = -1
